...
### fix
- **nfc_fuzzer**: Fixed a guaranteed crash on the Frame/NTAG/ISO15693 fuzzing profiles. `nfc_fuzzer_worker.c:218` called `nfc_listener_tx(nfc, tx_buf)`, passing the raw `Nfc*` handle where the SDK expects the `NfcListener*` instance (every other listener call in the function correctly used `listener`). The SDK dereferenced the wrong struct -> furi_check/hard fault on the first frame-level test case. Fixed to pass `listener`. (critical)
- **rayhunter_client**: Fixed a use-after-free on app exit. `rh_app_free()` freed the view dispatcher and views before `rh_uart_free()`, but `rh_worker_stop()` only nulls the UART RX callback pointer — the worker thread is joined inside `rh_uart_free()`. A UART line in flight could run `rh_worker_rx_line()` against freed `main_view`/`view_dispatcher`. Reordered `rh_uart_free()` (which joins the worker) before the view teardown. (high)
- **spi_flash_dump**: Fixed a start-up race that could falsely report a read/verify as failed. `worker_state` was set to Reading/Verifying before `spi_worker_start_read/verify()` — so the 250ms `worker_poll_timer_cb` could observe `!spi_worker_is_running() && state==Reading` during the start-up window, join the not-yet-started thread, and show "ERROR! Press Back" with the error tone while the dump actually succeeded in the background. `worker_state` is now set only after the worker thread is running. (medium)
- **rogue_ap_detector**: Fixed a stale threat indicator. On the RSSI-filter early-return path, `rogue_uart_line_cb()` pruned aged-out entries from the AP table but skipped `rogue_detect()`, so a cleared EVIL TWIN flag persisted in the scan view (and disagreed with the Results view) until an unfiltered line arrived. Added `rogue_detect()` before the filtered-line early-return. (medium)

### docs
- **Multi-app audit (bugs / incompatibility / memory)**: Fan-out source review of all 13 FAPs with adversarial verification. 5 confirmed defects (4 fixed above; the ccid_emulator 32-byte APDU-response truncation filed as a GitHub issue pending a cap-vs-reject design decision). Note: flipperpwn and hid_exfil could not be audited (finder agents blocked by a model safeguard) and remain unreviewed in this pass. dist/ .fap files for the 4 fixed apps were not rebuilt — ufbt is unavailable in this environment.
