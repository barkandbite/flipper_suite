...
### fix
- **ccid_emulator/card_parser.c**: Fixed FASC-N TLV length in the embedded `piv_card_content` CHUID response — `30 19` (length=25) claimed 25 bytes of FASC-N data but only 14 bytes followed before the `90 00` status word. Corrected to `30 0E` (length=14) so the inner TLV length matches the actual content. The embedded sample is auto-written to `/ext/ccid_emulator/cards/piv_card.ccid` on first run if absent, so any user installing the FAP fresh received the broken card. (Standalone `piv_emulator.ccid` was fixed 2026-05-20 in commit f8f7cac; the embedded version was missed because it's a shorter response constrained by `CCID_EMU_MAX_APDU_LEN=32` and uses a different layout.) Outer `53 10` (16 bytes) remains correct: now contains `30 0E` (2) + 14 bytes FASC-N = 16. Total response 20 bytes, fits in 32.
- **ccid_emulator/ccid_handler.h**: Corrected misleading docstring on `ccid_handler_start` — said "Switches USB to CCID mode with the chosen VID/PID preset" but the SDK's usb_ccid interface does not support custom VID/PID descriptors (documented in commit 7e63dca, 2026-04-13). The preset array is retained for future SDK support but is currently unused. Doc now reads "SDK default VID/PID" and reorders steps to reflect actual code (switch USB → set callbacks → insert card).

### docs
- **ccid_emulator**: Full re-trace review of all 1650 lines across 3 source files + 3 headers. PIV CHUID TLV fix above. Re-verified: all snprintf buffers (line[80] fits 63-char max in apdu_monitor_draw, atr_str[107] fits 33-byte ATR worst-case 104 chars, prefix[16], path[128], header[128], rules_str[32]), 4 views lifecycle correct (Submenu + Widget + custom View + VariableItemList, add/remove/free order correct), ViewModelTypeLocking on apdu_monitor (model mutation in draw safe under SDK-held lock), timer stopped before view freed, mutex freed after timer (teardown order: emulate→remove_views→timer→modules→dispatcher→card→paths→mutex→records), ring buffer indexing correct in draw/export/input (start_entry_idx wrap math verified: when wrapped, `(log_count + scroll_offset) % LOG_MAX_ENTRIES`), auto_scroll flag (added 2026-04-30) correctly disabled on Up press and re-enabled when Down reaches bottom, USB callback uses 5ms mutex timeout (drops log entry on contention, never blocks USB driver), hex/pattern parsers bounded (CCID_EMU_MAX_APDU_LEN=32, CCID_EMU_MAX_HEX_STR=96), rule_count capped at 24, CcidCard alloc+assert+memset, strncpy NUL-terminated, ccid_handler.c start/stop ordering correct for SDK+Momentum firmware (start: get_config→unlock→set_config→set_callbacks→insert; stop: remove→clear_callbacks→unlock→restore), `app->card` set before emulation starts and immutable during emulation (callbacks cleared in stop before any path that could modify it). All 3 embedded sample card TLV structures re-verified: test_card MasterFile (`6F 18 84 07 ... A5 0D ...`) ✓, PSE (`6F 1C 84 0E ... A5 0A ...`) ✓, PIV SELECT (`61 11 4F 06 ... 79 07 4F 05 ...`) ✓, PIV CHUID (`53 10 30 0E ...`) ✓ after fix, JavaCard ISD SELECT (`6F 10 84 08 ... A5 04 9F 65 01 FF`) ✓, GET STATUS (GP format, length-prefixed AID + lifecycle + privileges) ✓. Stack: GUI ~680/4096, USB callback ~70, timer ~100. SD card path uses `/ext/ccid_emulator/` (existing cross-app issue). Known characteristics unchanged: `discover_card_files` second `storage_dir_open` return unchecked (graceful degradation, no crash), `populate_card_browser_final` 0xFFFE Settings sentinel would collide if 65534+ cards present (impossible in practice).
