...
## 2026-08-10 — flipperpwn WiFi callback + exec-thread races

Full re-review of `flipperpwn` (8,237 lines across 9 files). Three real
concurrency defects fixed; `fap_version` (1,7)→(1,8), About string → v1.8.

### fix
- **flipperpwn**: Fixed a startup NULL-deref race in `fpwn_wifi_views_alloc`. The marauder log callback (`fpwn_wifi_rx_callback`) was registered right after the UART/marauder layer was allocated, but *before* `wifi_status_text`, `wifi_status_mutex`, and the status TextBox existed. The callback runs synchronously on the UART worker thread, and an ESP32 Marauder emits a prompt/banner the moment the serial port opens — so a line landing in that window called `furi_mutex_acquire(NULL)` (app is `memset` to 0) and hard-faulted the app on launch. The callback is now registered last, after its three resources exist, mirroring the teardown order that already deregisters it before freeing them.
- **flipperpwn**: Fixed a teardown race in the UART/marauder RX-callback setters. `fpwn_marauder_set_log_callback` and `fpwn_wifi_uart_set_rx_callback` published `ctx` before the function pointer — correct for registration but backwards for deregistration. On teardown the callbacks are cleared while the UART worker is still running (not joined until `fpwn_wifi_uart_free`), so the worker could pair a still-valid function pointer with the just-nulled `ctx` and call the UI callback with a NULL context → NULL `app` deref. The `if(cb) cb(...)` dispatch sites also read the pointer twice. Deregistration now clears the function pointer first, both callback/ctx fields are `volatile`, and each dispatch site reads the pointer once into a local.
- **flipperpwn**: Fixed a cross-run join deadlock. Leaving the Execute view was gated on the view model's `finished` flag, but the worker sets `finished=true` and then keeps running to write `last_run.txt` before sending `FPwnCustomEventExecDone` (which joins/frees the thread). Backing out in that window and starting another run let the old thread's queued `EXEC_DONE` join the *new* run's thread on the GUI thread; if the new run hit a `WAIT`/`WAIT_BUTTON` step, that thread spun on flags only the (now blocked) GUI thread could set → permanent deadlock, reboot required. The Execute-view Back is now gated on `exec_thread == NULL` (the thread actually reaped by the `ExecDone` handler on the same dispatcher thread). All three `finished=true` paths end by sending `EXEC_DONE`, so the view is never stranded.

### chore
- **flipperpwn**: `dist/flipperpwn.fap` NOT rebuilt this session — `ufbt` (and the Flipper SDK) are unavailable in the maintenance environment. The source fixes are pushed; the pre-built `.fap` must be regenerated by CI or a local `ufbt` build. All four changed files pass `clang-format` (the CI lint check).
