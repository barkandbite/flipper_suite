...
- **nfc_fuzzer**: Fixed frame/NTAG/ISO-15693 listener fuzzing passing the wrong handle to `nfc_listener_tx()` — it was called with the raw `Nfc*` instead of the `NfcListener*` instance. This is a type mismatch that fails the `-Werror` build; the app did not compile. (Consolidated from the daily-review backlog.)
...
### chore
- Reviewed the backlog of ~50 open draft maintenance PRs and consolidated the genuinely-unmerged, verified fixes here (nfc_fuzzer `nfc_listener_tx`, flipperpwn forward-decl + teardown race, rayhunter UAF, rogue_ap stale-threat, spi_flash_dump start-up race, `build_all.sh`; from PRs #26/#44/#58). The remainder are being closed as stale or superseded: PRs based on an outdated `main` that would revert merged work (#11, #13–#21), and the ccid_emulator cluster whose fixes already landed on `main` (PIV CHUID TLV, use-after-free-on-Back) or are superseded by the issue #59 fix above. Cosmetic ccid APDU-monitor display tweaks are deferred to a dedicated ccid_emulator session.
