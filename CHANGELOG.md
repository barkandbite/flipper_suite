...
## 2026-09-10 — CCID HAL migration, VID/PID, WIFI_CMD passthrough

### feat
- **ccid_emulator**: Migrated the USB CCID stack to survive official firmware ≥ API 88.0 (PR #4407), which removes CCID from the firmware HAL and requires apps to vendor it (issue #62). Vendored `ccid_usb.c`/`ccid_usb.h` (from OFW/RogueMaster reference commit `df4ee76a`) and added `ccid_compat.h`, which selects the HAL copy or the vendored copy at compile time via `__has_include(<furi_hal_usb_ccid.h>)` — the exact 1:1 signal for the 88.0 removal, since the header is physically deleted there. All call sites now use firmware-agnostic `ccid_compat_*` / `CCID_USB_INTERFACE`. The vendored file's whole body is guarded so it compiles to nothing on firmware that still ships CCID in the HAL (fbt globs every `.c`), keeping the current API-87.1 build byte-for-byte behavior-identical. Bumped `fap_version` (1,0)→(1,1).
- **ccid_emulator**: Implemented USB VID/PID customization (issue #4, previously deferred as SDK-unsupported). On the new stack, `FuriHalUsbCcidConfig{vid,pid,manuf,product}` is passed to `furi_hal_usb_set_config`, so the **USB Device** settings selector (Default / Generic Reader / Yubikey) now drives the emulated reader's descriptor. On old firmware the HAL exposes no config hook, so the selector is inert and the settings screen shows a "VID/PID fixed on FW" note. Restored the preset selector UI that had been reduced to a static label while it was non-functional.
- **flipperpwn**: Added the `WIFI_CMD <raw>` payload command, sending an arbitrary Marauder CLI command verbatim to the ESP32 over the UART bridge (`fpwn_marauder_send_raw`). `$VAR` expansion applies as in `STRING`. Fills the gap where only a fixed set of `WIFI_*` wrappers existed. Bumped `fap_version` (1,7)→(1,8).
- **evil_portal**: Rewrote all five `marauder_scripts/fpwn/*.fpwn` modules so they actually execute. Two problems, both fixed: (1) they never sent any Marauder attack command — now they do, via `WIFI_CMD` (`select -a`/`attack -t deauth`, `sniffpmkid`, `sniffprobe`, `scansta`, `evilportal -c set/start`, `blespam -t apple|windows|samsung`); and (2) more fundamentally, they were structured as several interleaved `PLATFORM WIN/MAC/LINUX` blocks with global `WIFI_*` lines between them, but the FlipperPwn executor runs exactly **one** platform section (the OS match, or `PLATFORM ALL` fallback) and skips everything else — so most lines, including the WiFi commands, never ran on any target. Restructured each into a single `PLATFORM ALL` block using OS-aware commands (`OPEN_NOTEPAD`, `PRINT`, `LED_COLOR`) and an `IF_CONNECTED` guard, matching the working `wifi_full_attack.fpwn` pattern. Also dropped multi-word `OPTION` defaults (`HASHCAT_CMD`, quoted `SITE_NAME`/`ASSESSOR`) that the single-token option parser would have truncated.

### docs
- **flipperpwn/README.md**, **ccid_emulator/README.md**, **evil_portal/marauder_scripts/README.md**: Documented `WIFI_CMD`, the VID/PID selector + firmware caveat, and the scripts' dependency on FlipperPwn 1.8. Noted that Marauder command names/flags vary by firmware fork and should be adjusted to the flashed build.

### chore
- **Build**: `ufbt` is not available in this environment; changes were verified by static trace and rely on the repo CI matrix for the compile. The new CCID stack's `CCID_USB_CONFIG_SUPPORTED` path cannot be CI-built until the pinned SDK moves to API ≥ 88.0; the API-87.1 path is unchanged and remains the built/shipped one. `dist/` `.fap`s are therefore left unrebuilt this session (documented in TODO).
