...
- **evil_portal**: Wired all five `marauder_scripts/fpwn/*.fpwn` modules to actually launch their attacks via `WIFI_CMD` (`select -a`/`attack -t deauth`, `sniffpmkid`, `sniffprobe`, `scansta`, `evilportal -c set/start`, `blespam -t apple|windows|samsung`). Previously they described attack sequences in comments and typed status text via HID but never sent a single attack command to the ESP32 — they were non-functional as attack tools until FlipperPwn gained a raw-command path.
