...
### docs
- **ccid_emulator/README.md**: Fixed inaccurate "Sample Profiles" and SD card layout sections. The README described `test_card.ccid` as a PIV-style card (matching the external repo sample) but the *auto-created* `test_card.ccid` is actually EMV-style (Visa SELECT, PSE, GPO, READ RECORD) — different content under the same filename. README also referenced `piv_emulator.ccid` in the SD card layout, but the app auto-writes `piv_card.ccid` and `javacard.ccid` (neither documented). Rewrote section to clearly distinguish the three auto-created samples (`test_card.ccid`, `piv_card.ccid`, `javacard.ccid`) from the two optional richer profiles in `ccid_emulator_sample_cards/` (`test_card.ccid`, `piv_emulator.ccid`). Added `logs/` directory to the layout diagram (created on demand by Right-press export).
- **ccid_emulator**: Full re-trace review of all 1650 lines across 3 source files + 3 headers. No bugs found. ccid_emulator.c (4 views lifecycle correct: Submenu + Widget + custom View + VariableItemList, ViewModelTypeLocking on apdu_monitor, lock ordering view_model→log_mutex consistent across draw/input/timer/custom-event paths — no deadlock, timer stopped before view free, mutex freed after timer, all snprintf buffers verified — line[80] fits "%lu C>%.50s" worst case 63 chars, header[128] fits 80 chars, path[128] fits 53 chars, prefix[16] fits 13 chars, atr_str[CCID_EMU_MAX_ATR_LEN*3+8]=107 fits 33×3+5=104, rules_str[32] fits 14 chars, scroll-bar division-by-zero guarded by `total > visible_lines` branch, ring-buffer indexing in draw/export/input correct for both pre-wrap and post-wrap cases, auto_scroll flag toggled correctly on Up/Down, refresh timer skips work when log_dirty is false). card_parser.c (parse_hex_string and parse_hex_pattern bounded by out_max, line[256]+sec_name[32] stack-safe at ~300B, strip() empty-string guard correct, parse_rule_line cmd_buf[96]+resp_buf[96] stack-safe, rule_count capped at 24, all strncpy NUL-terminated, write_sample_file checks file existence before creating). ccid_handler.c (USB callback acquires log_mutex with 5ms timeout — drops log on contention, never stalls USB driver, ATR clamped to CCID_EMU_MAX_ATR_LEN, response clamped to CCID_MAX_DATA_BLOCK_LEN, start/stop ordering correct for SDK+Momentum: configure callbacks struct before switching USB, clear callbacks BEFORE switching USB back). All 3 embedded sample cards' TLV re-verified correct (test_card MasterFile/PSE/GPO/READ RECORD, PIV SELECT/CHUID, JavaCard ISD/GET STATUS GP-compact format). Stack: GUI ~680/4096, USB callback ~80, timer ~100. No new cross-app issues. Prior fixes verified: auto_scroll flag (2026-04-30), APDU monitor MAX_VISIBLE=3 (2026-04-17), TLV lengths in embedded test_card (2026-04-17), PIV CHUID TLV in external piv_emulator.ccid (2026-05-20), log_count uint32 (2026-04-06), malloc NULL check (2026-04-06).
