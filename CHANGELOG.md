...
- **ccid_emulator/ccid_emulator.c**: Fixed APDU monitor clipping the newest response line. With `APDU_MON_MAX_VISIBLE=3`, the 3rd entry's `C>` line drew at y=53 and then `y` reached 63, so `if(y > 62) break` fired before the `R>` line could draw. Under auto-scroll the 3rd entry is the newest exchange, so the most recent APDU's response was never visible. Raised the break threshold to `>63` (baseline 63 is the last on-screen row and the hex/`C`/`R`/`>`/`*` glyphs have no descenders). Surfaced by parallel daily-maintenance branches (kCvXf, 0Ksjq, cBgvD); the 2026-05-27 trace missed it.
- **ccid_emulator_sample_cards/piv_emulator.ccid**: Fixed CHUID response truncated past the 32-byte APDU cap. The response was 62 bytes, but `parse_hex_string` caps each rule response at `CCID_EMU_MAX_APDU_LEN` (32) and silently drops the rest, so on load it became `53 3A` (claims 58 content bytes) with only 30 following — a malformed TLV. The 2026-05-20 fix corrected the inner FASC-N length but not the overall overrun. Shrunk to a valid 30-byte CHUID (`53 1A` container: FASC-N `30 0E`, GUID `34 04`, empty signature `3E 00`, empty error-detection `FE 00`). Surfaced by the maintenance/ccid-emulator-2026-05-21 branch.
...
- **branch review**: Audited all 18 non-`main` branches for mergeability. 6 merge cleanly (the 5 `claude/charming-*` sessions + `maintenance/ccid-emulator-2026-05-21`); 8 conflict (the `badusb-pro`/`hid-exfil` families, all ~19 commits behind and superseded by fixes already in main); 3 are stale/divergent (`uart-sniff-ring-fix`, `fix/uart-sniff-ring-read-order`, `daily-review-2026-04-22` — ~160 commits ahead on an old base, do not merge). Four clean branches plus this one all independently reviewed ccid_emulator and overlap heavily (they conflict with each other despite each merging cleanly to main). Their collective findings — the APDU monitor clipping and the standalone CHUID truncation — are now folded into this branch, which supersedes them. `claude/charming-mccarthy-ZFWYp` is distinct and contains likely build fixes for other apps (flipperpwn forward declaration for the 2026-05-08 change, `variable_item_list_set_header` removal in 3 apps, `build_all.sh` `((PASS++))`-under-`set -e` abort) — flagged for separate verification/merge.
