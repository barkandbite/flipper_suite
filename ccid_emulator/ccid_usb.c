/* ccid_usb.c — app-vendored USB CCID device stack.
 *
 * Source: flipperzero-firmware (OFW PR #4407) / RogueMaster wPlugins commit
 * df4ee76a, which moved the CCID USB layer out of the firmware HAL and into the
 * app.  Vendored here verbatim so ccid_emulator keeps working on firmware where
 * the HAL no longer provides CCID (API >= 88.0).  See issue #62.
 *
 * Because fbt/ufbt compiles every .c in the app directory, this file is also
 * fed to the compiler on OLDER firmware that STILL ships CCID in the HAL — where
 * its symbols would be redundant and its use of the newer USB device internals
 * may not even build.  The whole body is therefore compiled only when the HAL
 * CCID header is absent (the exact 1:1 signal for the API 88.0 removal). */
#if !(defined(__has_include) && __has_include(<furi_hal_usb_ccid.h>))
...
#endif /* CCID vendored only when the HAL lacks it (issue #62) */
