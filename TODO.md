...
- **flipperpwn**: Reviewed 2026-04-01. Fixed EXFIL_USB Windows COM port filtering (parity with os_detect.c CDC fix). Race condition in `fpwn_marauder_get_*` accessors — 14 unsafe call sites: 5 in `wifi_views.c:fpwn_wifi_save_results` (lines 986-1064), 9 in `payload_engine.c` WIFI_* commands (lines 1468-1908). Safe `fpwn_marauder_copy_*` functions already exist (marauder.c:810-858) and are used by view model draw code (5 call sites). **Recommended fix**: migrate all 14 callers to `copy_*` with `malloc`/`free` temporary buffers (not stack — arrays are up to 128×sizeof(FPwnPortResult)). Cannot use lock/unlock wrapper because mutex is FuriMutexTypeNormal (non-recursive), and `get_*` functions internally acquire. Recursive mutex is a code smell. Low practical impact (scans stopped before save/use), but technically a data race. Schedule for next flipperpwn review (due ~2026-04-13).
